...
async function fetchComments(worldId: string, page: number = 1) {
  const res = await fetch(`/api/worlds/${worldId}/comments?page=${page}&page_size=20`);
  const data = await res.json();
  return data;
}

async function submitComment(worldId: string, content: string) {
...
export default function WorldCommentsPage({ params }: { params: { id: string } }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (data.code === 200) { setNewComment(""); setPage(1); }
  };
    ...
    <div className="mx-auto max-w-2xl py-8">
      <div className="sf-card">
        <div className="border-b border-[#DCE9FF] p-4">
          <h1 className="text-xl font-semibold text-[#1F2A44]">世界评论</h1>
          <p className="text-sm text-[#5B6B8C] mt-1">分享你对这个世界的看法</p>
        </div>
        <div className="p-4">
          <form onSubmit={handleSubmit} className="mb-6">
            <textarea value={newComment} onChange={(e) => setNewComment(e.target.value)}
              placeholder="写下你的评论..." className="sf-input resize-none" rows={3} />
            <div className="flex justify-end mt-2">
              <button type="submit" disabled={!newComment.trim()} className="sf-btn-primary disabled:opacity-50">发布评论</button>
            </div>
          </form>
          {loading ? (
            ...
            <div className="space-y-4">
              {comments.map((comment) => (
                <div key={comment.id} className="rounded-xl bg-[#F8FBFF] p-4 hover:bg-[#EEF6FF] transition-colors">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-[#5B6B8C]">{new Date(comment.createdAt).toLocaleDateString()}</span>
                      </div>
                      ...
                      <div className="flex items-center gap-4 mt-3">
                        ...
                        <button className="flex items-center gap-1 text-xs text-[#5B6B8C] hover:text-[#1F2A44]">
                          <span>💬</span><span>{comment.replyCount}</span>
                        </button>
                      </div>
                    </div>
                  </div>
